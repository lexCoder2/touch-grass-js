...
export function buildShareText(data: ShareData): string {
  const tpl = socialTemplates[Math.floor(Math.random() * socialTemplates.length)];
  return tpl
    .replace('{count}', String(data.count ?? 0))
    .replace('{streak}', String(data.currentStreak ?? 1));
}

export function buildShareUrl(platform: 'twitter' | 'linkedin', text: string): string {
  const encoded = encodeURIComponent(text);
  if (platform === 'twitter') {
    return `https://twitter.com/intent/tweet?text=${encoded}`;
  }
  const repo = encodeURIComponent('https://github.com/lexCoder2/touch-grass-js');
