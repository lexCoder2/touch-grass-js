...
import { getArt, getRandomMessage, buildShareText, buildShareUrl, ArtScene } from './data';
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('touchGrass.go', () => cmdTouchGrass(context)),
    vscode.commands.registerCommand('touchGrass.stats', () => cmdShowStats(context)),
    vscode.commands.registerCommand('touchGrass.share', () => cmdShare(context)),
    vscode.commands.registerCommand('touchGrass.reset', () => cmdReset(context))
  );
...
function cmdTouchGrass(context: vscode.ExtensionContext): void {
  ...
  updateStatusBar(context);
  openGrassPanelOrReveal(context, lastArt, lastMessage, streakResult, outdoorMinutes);
}
...
async function cmdShare(context: vscode.ExtensionContext): Promise<void> {
  const platform = await vscode.window.showQuickPick(
    [
      { label: '🐦 Twitter / X',   description: 'Opens twitter.com',   value: 'twitter'   },
      { label: '💼 LinkedIn',       description: 'Opens linkedin.com',   value: 'linkedin'  },
      { label: '📸 Instagram',      description: 'Copies text to clipboard', value: 'instagram' },
    ],
  ...
  if (platform.value === 'instagram') {
    ...
    vscode.window.showInformationMessage('📋 Copied to clipboard! Paste it into your Instagram caption.');
    return;
  }
...
async function cmdReset(context: vscode.ExtensionContext): Promise<void> {
  ...
  if (confirm === 'Reset') {
    ...
    updateStatusBar(context);
    vscode.window.showInformationMessage('🌱 Streak reset. Fresh start — go touch some grass.');
  }
}
...
function startReminderTimer(context: vscode.ExtensionContext): void {
  ...
  const minutes: number = Math.max(5, config.get('reminderIntervalMinutes', 60));
...
function updateStatusBar(context: vscode.ExtensionContext): void {
  ...
  // Label
  statusBarItem.text = d.currentStreak > 0
